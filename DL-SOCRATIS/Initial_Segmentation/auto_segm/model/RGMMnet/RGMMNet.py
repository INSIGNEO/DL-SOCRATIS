...
from keras.layers import Layer, Dense, Flatten, Activation, LeakyReLU, Softmax, ELU
...
class RGMMNet(Layer):
  ...
  def __init__(self, u, d, n_mixture_models, n_hidden, batch_size, **kwargs):
    self.u = u
    self.d = d
    mm_case='RGMM'
    self.case=mm_case
    ...
    if mm_case=='GMM':
      for k in range(self.n_gaussian):
        weighting = Gaussian_Weighting(self.u, self.d,batch_size )
        self.weightings.append(weighting)

    if mm_case=='RMM':
      for k in range(self.n_rician):
        weighting = Rician_Weighting(self.u, self.d)
        self.weightings.append(weighting)
    ...
    if mm_case=='RGMM':
      for k in range(self.n_gaussian):
        ...
        self.weightings.append(weighting)
      for k in range(self.n_rician):
        weighting2 = Rician_Weighting(self.u, self.d,batch_size )
  ...
  def call(self, X):
    ...
    if self.case=='RGMM':
      ...
      batch_size, n_nodes, n_features = X.shape
      batch_size=self.batch_size
      weights = tf.reshape(weights, [batch_size*n_nodes, n_features*(self.n_gaussian+self.n_rician)])
