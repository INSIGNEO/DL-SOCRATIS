//refernece
//EM ALGORITHM FOR ESTIMATING THE PARAMETERS OF A
//MULTIVARIATE COMPLEX RICIAN DENSITY FOR POLARIMETRIC
//SAR
//Thomas L. Marzetta
//Nichols Research Corporation
//251 Edgewater Drive, Wakefield, MA 01880
 ...
 *
 *=========================================================================*/
#ifndef itkRicianMembershipFunction_hxx
#define itkRicianMembershipFunction_hxx
...
RicianMembershipFunction< TMeasurementVector >::RicianMembershipFunction()
{
...
template< typename TMeasurementVector >
void RicianMembershipFunction< TMeasurementVector >::PrintSelf(std::ostream & os, Indent indent) const
{
...
template< typename TMeasurementVector >
void RicianMembershipFunction< TMeasurementVector >::SetMean(const MeanVectorType & mean)
{
  if ( this->GetMeasurementVectorSize() )
    {
    MeasurementVectorTraits::Assert(mean,
                                    this->GetMeasurementVectorSize(),
                                    "RicianMembershipFunction::SetMean(): Size of mean vector specified does not match the size of a measurement vector.");
    }
  else
    {
...
template< typename TMeasurementVector >
void RicianMembershipFunction< TMeasurementVector >::SetCovariance(const CovarianceMatrixType & cov)
{
  // Sanity check
  ...
  if( m_CovarianceNonsingular )
    ...
    // calculate coefficient C of multivariate Rician
    m_PreFactor =
      1.0 / ( det *
        std::pow( itk::Math::pi,
...
template< typename TMeasurementVector >
inline double RicianMembershipFunction< TMeasurementVector >::Evaluate(const MeasurementVectorType & measurement) const
{
...
vnl_matrix_inverse< double > inv_cov( m_Covariance.GetVnlMatrix() );
  ...
  for(MeasurementVectorSizeType r = 0; r < measurementVectorSize; ++r){
    ...
    for (MeasurementVectorSizeType c = 0; c < measurementVectorSize; ++c){
      rowdot += (measurement[c]*m_InverseCovariance(r, c)*measurement[r]) + (m_Mean[c]*m_InverseCovariance(r, c)*m_Mean[r]); 

//////////////////////////////////////////////////    
      // compute Bessel z value of first kind)
       z+=2*std::abs(m_Mean[c]*m_InverseCovariance(r, c)*measurement[r]);

///////////////////////////////////////////////////////

     } //end c for
   }//edn r for 
   if (z>200){z=200;} //upper limit
  Besel_o = boost::math::cyl_bessel_i(0, z);
  temp = std::exp(-rowdot)*Besel_o; 
  //temp+=temp*measurement[0];  
...
//std::cout<<"Besel_o= "<<Besel_o<<std::endl;
//std::cout<<"z= "<<z<<std::endl;
//std::cout<<"rowdot= "<<rowdot<<std::endl;
//std::cout<<"temp= "<<temp<<std::endl;
//std::cout<<"m_PreFactor= "<<m_PreFactor<<std::endl;
//std::cout<<"m_InverseCovariance= "<<m_InverseCovariance(0, 0)<<std::endl;
//std::cout<<"m_Mean= "<<m_Mean[0]<<std::endl;
//std::cout<<"measurement[0]= "<<measurement[0]<<std::endl;
//std::cout<<"approach= "<<m_PreFactor * temp<<std::endl;
//std::cout<<"/////////////////////////"<<std::endl;
...
typename LightObject::Pointer RicianMembershipFunction< TVector >::InternalClone() const
{
